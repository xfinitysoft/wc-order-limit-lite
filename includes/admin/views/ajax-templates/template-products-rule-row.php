...
<tr class="wcol-new" data-id="<?php echo $id  ?>">
	<td class="check-column wcol-cb">
		<input type="checkbox"/>
		<input type="hidden" name="wcol-rules[product-rules][rule-id][<?php echo esc_html($_POST['wcol_pid']); ?>]" value="<?php echo $id  ?>">
	</td>
	<td class="column-primary">
		<select class="wcol-select-products" name="wcol-rules[product-rules][object-ids][<?php echo esc_html($_POST['wcol_pid']); ?>][]" multiple="multiple"></select>
	...
	<td data-colname="<?php esc_attr_e('Minimum Limit', 'xsollwc-domain'); ?>">
		<input type="number" min="0" class="wcol-rule-min-limit" name="wcol-rules[product-rules][rule-limit][<?php echo esc_html($_POST['wcol_pid']); ?>]" value="0" />
	</td>
	...
	<td data-colname="<?php esc_attr_e('Applied On', 'xsollwc-domain'); ?>">
		...
		?>
		<select class="wcol-select-applied-on" name="wcol-rules[product-rules][applied-on][<?php echo esc_html($_POST['wcol_pid']); ?>]" >
			<?php echo $applied_on_ajax_options; ?>
		</select>
	</td>
	<td data-colname="<?php esc_attr_e('Accumulatively', 'xsollwc-domain'); ?>">
		<input type="hidden" class="wcol-loop-checkbox-hidden" name="wcol-rules[product-rules][accomulative][<?php echo esc_html($_POST['wcol_pid']); ?>]"/>
			...
			<div class=" wcol-rule-options wcol-hidden">
				...
				<table class="">
					<tr>
						...
						<td>
							<input type="hidden" class="wcol-loop-checkbox-hidden" name="wcol-rules[product-rules][disable-limit][<?php echo esc_html($_POST['wcol_pid']); ?>]" />
					...
					<tr>
						...
						<td>
							<input type="hidden" class="enable-max-rule-limit-hidden wcol-loop-checkbox-hidden" name="wcol-rules[product-rules][enable-max-rule-limit][<?php echo esc_html($_POST['wcol_pid']); ?>]"/>
					...
					<tr class="wcol-hidden">
						...
						<td><input type="number" min="0" class="wcol-rule-max-limit" name="wcol-rules[product-rules][max-rule-limit][<?php echo esc_html($_POST['wcol_pid']); ?>]" /></td>
					</tr>
